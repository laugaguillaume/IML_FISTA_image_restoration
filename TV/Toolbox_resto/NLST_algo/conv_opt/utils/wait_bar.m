function h = wait_bar(X,varargin)
...
% 1 - SET WINDOW SIZE AND POSITION
    ...
    winheight = 90;                             % Height of timebar window
    winpos = [0.5*(screenwidth-winwidth), ...
              0.5*(screenheight-winheight), ...
              winwidth, winheight];             % Position
...
wincolor = 0.85*[1 1 1];
    ...
    userdata.text(1) = uicontrol(h,'style','text','hor','left',...
        'pos',[10 winheight-27 winwidth-20 20], 'string',message,...
        'FontSize',10,'backgroundcolor',wincolor,'tag','message');

    % 2.3 - Build estimated remaining static text textbox
    est_text = 'Remaining time: ';
    userdata.text(2) = uicontrol(h,'style','text','string',est_text,...
        'pos',[10 17 winwidth/2 20],'FontSize',8,...
...
actual = get(userdata.bar,'xdata');
if etime(clock,userdata.inc) > 1 || progress == 1 || (progress ~= 0 && all(actual==0))
    ...
    set(userdata.remain,'string',r_mes);                                        % Update remaining time
    set(userdata.elapse,'string',e_mes);                                        % Update elapsed time
    set(userdata.percent,'string', strcat(num2str(floor(100*progress)),'%'));   % Update progress
    set(h,'Name',[num2str(floor(100*progress)) '% Complete']);                  % Update figure name

end

% 3 - FLUSH THE EVENT QUEUE (Needed for window to update)
drawnow;