 function [x,x2, it, time, norm_xy, varargout] = CV_algo_uncertainties(x_init, f, g, h, opt, varargin)
...
for it = 1:opt.iter
    ...
    %[p,p2] = f.prox(x_hat, x2_hat, gamma);
     val_inv = inv([(1+tau), -1;-1 (1+tau)]);
    ...
    v2     = v2_hat - gamma * project_S(v2_hat/gamma, opt);    
    ...
    fprintf('norme :%3.2f\t crit_real_data = %3.2f\t crit_real_pen = %3.2f\n',norm(x-x2,'fro')^2,varargout{1}(it),varargout{2}(it));
